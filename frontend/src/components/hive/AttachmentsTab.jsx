//frontend\src\components\hive\AttachmentsTab.jsx
...
export default function AttachmentsTab({ list, loading, details, q = "" }) {
  ...
  const rowsRaw = useMemo(() => {
    ...
    (list || []).forEach((s) => {
      ...
      const items = s.attachmentsRows || s.details?.attachments || [];
      items.forEach((r) => {
        out.push({
          value: r.value ?? 0,
          first_name:
  ...
  if (!details) {
    return (
      <div className="empty">
        Turn on <strong>Include details</strong> to see Attachments rows.
      </div>
    );
  }
  if (loading) return <div className="loading">Loading…</div>;
  if (!rows.length) return <div className="empty">No attachment rows.</div>;
